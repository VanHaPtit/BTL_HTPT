...
Dự án dùng mô hình máy chủ làm trung tâm (Server-authoritative):

- Mỗi khi bạn gõ một chữ, máy tính sẽ đóng gói chữ đó kèm số thứ tự (phiên bản) và gửi lên Server.
- Server nhận được sẽ tạm khóa tài liệu lại. Nếu có ai đó vừa gửi chữ lên trước bạn, Server sẽ dùng thuật toán OT để tính toán lại vị trí chữ của bạn cho đúng. Cuối cùng, Server tăng số phiên bản lên, lưu vào MySQL và gửi kết quả về cho mọi người trong phòng cập nhật màn hình.
...
* **Cập nhật trạng thái hiện diện (Presence Update):** Đồng bộ tọa độ con trỏ (cursor) và vùng chọn (selection) của người dùng qua kênh `/app/documents/{id}/presence.update`.
...
* *(Lưu ý: Nếu máy có cài sẵn Docker, bạn có thể bỏ qua bước cài MySQL và Redis ở trên)*
